...
import org.apache.sysds.runtime.codegen.SpoofLLVM;
...
public class SpoofCompiler {
	...
	public static void loadNativeCodeGenerator(GeneratorAPI generator) {
		...
		if(!native_contexts.containsKey(generator)) {
			...
			if(generator == GeneratorAPI.LLVM){
				String llvmLibAbsolutePath = System.getProperty("java.library.path");
				String llvmVersion = "LLVM-10"; // TODO: understand what files are needed to load to use LLVM

				boolean isLoaded = NativeHelper.loadLLVM(llvmLibAbsolutePath, llvmVersion, "");

				if(!isLoaded)
					isLoaded = NativeHelper.loadLLVM(null, llvmVersion, "");

				if(isLoaded) {
					boolean llvm_context = true; //initialize_llvm_context(); // TODO: JNI native call: ;
					if (llvm_context){
						native_contexts.put(GeneratorAPI.LLVM, 10L); // TODO: ask what long add to the hashmap
						API = GeneratorAPI.LLVM;
						LOG.info("Successfully loaded spoof llvm library");
					}
					else {
						API = GeneratorAPI.JAVA;
						LOG.error("Failed to initialize spoof llvm context. Falling back to java codegen\n");
					}
				}
				else {
					API = GeneratorAPI.JAVA;
					LOG.error("Loading of spoof native llvm failed. Falling back to java codegen\n");
				}
			}
		}
	}
	...
	public static void unloadNativeCodeGenerator() {
		...
		if(native_contexts.containsKey(GeneratorAPI.LLVM)){
			//destroy_llvm_context(); // TODO: call native method
			native_contexts.remove(GeneratorAPI.LLVM);
	...
	// TODO: understand if something specific need to be done/pass like in compile_cuda_kernel()
	private static boolean compile_llvm(String name, String src){return true;};

	private static native boolean initialize_llvm_context();

	private static native void destroy_llvm_context();
		...
		try
		{
			...
			for( Entry<Long, Pair<Hop[],CNodeTpl>> cplan : cplans.entrySet() )
			{
				...
				if( cla == null ) {
					...
					if(API == GeneratorAPI.LLVM){
						if(tmp.getValue().isSupported(API)) {
							src_llvm = tmp.getValue().codegen(false, GeneratorAPI.LLVM);
							native_compiled_successfully = compile_llvm(tmp.getValue().getVarname(), src_llvm);
							if(native_compiled_successfully)
								CodegenUtils.putNativeLLVMOpData(new SpoofLLVM(tmp.getValue()));
							else{
								LOG.warn("LLVM compilation failed, falling back to JAVA");
								tmp.getValue().setGeneratorAPI(GeneratorAPI.JAVA);
							}
						}
						else
							LOG.warn("CPlan " + tmp.getValue().getVarname() + " not supported by SPOOF LLVM");
					}
